...
# Module Introduction: This module contains the relevant processes for sample handling, and any modifications related to sample handling can be implemented here.
...
from sample_processor.sgame_expr import SgameExpr as RLDataInfo
from conf.config import Config, ModelConfig
...
class SgameSampleProcessor(SampleProcessor):
    """
    Class related to sample handling
    ...
    def __init__(self):
        ...
        # aisrv requires must_need_sample_info for sample generation.
    ...
    def gen_expr(self, must_need_sample_info, network_sample_info, sample_model_version=None, current_model_version=None):
        ...
        Generate a sample:
        Args:
            must_need_sample_info: Returned by the on_update function of the run_handler.
            network_sample_info: The Actor prediction returns the action and network parameters. 

        生成一个样本
        Args:
            must_need_sample_info: 由run_handler的on_update函数返回
            network_sample_info: Actor预测会返回action和网络参数
        """
        ...
        for i in range(self.num_agents):
            ...
            # select sample
            # 删选样本
            if not sample_model_version or not current_model_version:
                self.save_sample(**sample, agent_id=i, game_id=self.game_id, uuid=None)
            else:
                if current_model_version == sample_model_version:
                    self.save_sample(**sample, agent_id=i, game_id=self.game_id, uuid=None)
    ...
    def proc_exprs(self, del_last=False):
        """
        Generate full sample of an Episode

        生成一个Episode的全量样本
        """
        total_frame_cnt = len(self.must_need_sample_info)

        # In exceptional cases, it is necessary to delete the last saved sample to ensure the correctness of the samples.
        ...
        for agent_data in train_data:
            for sample in agent_data:
                train_data_all.append({
                    # When sending samples, enforce conversion to float16.
                    # 发送样本时, 强制转换成float16
                    'input_datas': np.array(sample[1], dtype=np.float16)
                })

                # priority of each sample
                # 每个样本的优先级设置
                train_data_prioritized.append(1.0)
        ...
        return train_data_all, train_data_prioritized, train_frame_cnt, return_rew
    ...
    def save_sample(self, frame_no,
        ...
        # just need to report reward sum while id=0
    ...
    def _calc_reward(self):
        ...
        使用GAE（Generalized Advantage Estimation）计算累积奖励和优势值。
        参数：
            reward_sum：用于值函数损失（value loss）的累积奖励。
            advantage：用于策略损失（policy loss）的优势值。
        """
        for i in range(self.num_agents):
    ...
    def _format_data(self):
        ...
        for i in range(self.num_agents):
            ...
            # Do not discard the tail sample
            # 不丢弃尾部样本
            if cnt>0:
    ...
    def _send_game_data(self):
        """
        send game info like: [task_id: int, task_uuid: str, game_id: str, frame_no: int, real_data: data in str]

        发送对局信息,格式如：[task_id: int, task_uuid: str, game_id: str, frame_no: int, real_data: data in str]
        """
        all_samples = []
        
        # Do not save samples from training with the old model during self-play.
