...
from framework.common.utils.common_func import TimeIt, is_list_eq, list_diff, set_schedule_event, \
compress_data, decompress_data, actor_learner_aisrv_count, get_host_ip, get_uuid, Context, make_single_dir, python_exec_shell
...
class AiServer(multiprocessing.Process):
    ...
    '''
    aisrv从alloc服务获取arena的IP地址
    '''
    def get_arena_ip_from_alloc(self):
        ...
        while retry_num < CONFIG.socket_retry_times:
            if not int(CONFIG.self_play):
                arena_address = self.alloc_util.get_arena_ip(CONFIG.set_name, CONFIG.self_play_set_name)
        ...
        if retry_num >= CONFIG.socket_retry_times:
            self.logger.error(
                f'AiServer server get arena address retry times more than {CONFIG.socket_retry_times}, will start with configure file')
            return None
    ...
    '''
    启动aisrvhandler, 步骤如下:
    1. 从alloc获取arena地址, 否则默认使用本地地址
    2. 根据arena地址的个数, 启动AisrvHandler进程
    '''
    def start_aisrv_handler(self):
        '''
        
        aisrv在启动时, 从alloc进程获取actor和learner的分配IP地址; 如果不从alloc访问默认就是本地127.0.0.1地址
        1. actor, learner的分配IP地址
        2. arena的分配IP地址
        '''
        arena_address = [f'{KaiwuDRLDefine.LOCAL_HOST_IP}:{CONFIG.arena_svr_port}']
        if int(CONFIG.use_alloc):
            ...
            if not registry_result:
                ...
                arena_address = self.get_arena_ip_from_alloc()

        if not arena_address:
            self.logger.error(f'AiServer fail to get arena address')
            return
        ...
        self.simu_ctx.get_active_handlers_model_version_value = self.get_active_handlers_model_version_value.__get__(self)
        ...
        # 针对返回来的arena的IP列表, 每个IP启动单个进程
        for address in arena_address:
    ...
    def before_run(self):
        ...
        self.logger.info(f'AiServer is start at {CONFIG.aisrv_ip_address}:{CONFIG.aisrv_server_port}, pid is {os.getpid()}, run_mode is {CONFIG.run_mode}, self_play is {CONFIG.self_play}')
...
class AiSrvHandle(multiprocessing.Process):
    ...
    def __init__(self, arena_address, simu_ctx) -> None:
        super(AiSrvHandle, self).__init__()
        self.arena_address = arena_address
        self.simu_ctx = simu_ctx

        # 建立zmq通信
        '''
        self.client_id = get_uuid()
        parts = self.arena_address.split(":")
        ip = parts[0]
        port = parts[1]
        ...
        self.simu_ctx.client_address = self.arena_address
        ...
        self.logger.info(f'aisrvhandle established connect to {self.arena_address}, slot id is {str(self.slot_id)}, min_slot_id is {self.min_slot_id}')
    ...
    def episode_stop(self):
        ...
        self.logger.info('aisrvhandle lost connection from {}', str(self.arena_address))
