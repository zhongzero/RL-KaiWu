...
# Module Introduction: This module contains the implementation of network architectures,
# and users can implement their own custom networks within this module.
...
from conf.config import DimConfig


class Model():
    '''
    Network Structure Definition: The network structure is utilized by both the actor and learner, differentiated by the mode.

    网络结构定义, actor和learner都会使用到, 用mode进行区分。
    '''

    def __init__(self, mode="actor"):
        """
        Initialization of the model.feature parameter configure.
        ...
        # Reading the configuration information of the model.
        # 读取模型的配置信息
        ...
        # The actor only performs inference and does not train. Certain configurations need to be overridden.
    ...
    def build_graph(self, datas, update):
        ...
        return loss, {'all_loss_value': loss, 'value_loss': self.value_cost, 'policy_loss': self.policy_cost, 'entropy_loss': self.entropy_cost,
                      'ratio_mean': ratio_mean, 'policy_log_p_mean': policy_log_p_mean, 'old_policy_log_p_mean': old_policy_log_p_mean}
    ...
    def _calculate_loss(self, unsqueeze_label_list, old_label_probability_list, fc2_label_list, unsqueeze_reward, unsqueeze_advantage, fc2_value_result, seri_vec, unsqueeze_is_train, unsqueeze_weight_list):
        ...
        for task_index in range(len(self.is_reinforce_task_list)):
            if self.is_reinforce_task_list[task_index]:
                ...
                # The intermediate process of handling the mask for legal action.
                # 对legal action掩码的中间过程
    ...
    def _inference(self, seri_vec, init_lstm_cell, init_lstm_hidden, only_inference=False):
        '''
        The specific network architecture definition, for more details, 
        please refer to the document: "Honor of King 1v1 Experimental Development Guide - Environment Introduction - Network Model".

        具体的网络结构定义，详情可以查看文档: “王者荣耀1v1实验开发指南 - 环境介绍 - 网络模型“
        '''
        # model design
        # 模型设计
        if only_inference: 
            # for actor, input seri_vec as (feature, legal_action), legal_action size differ from learner's.
            # 对于 Actor，输入 seri_vec 为 (feature, legal_action)，其中 legal_action 的大小与learner的不同。
