...
# Module Introduction: This module includes the implementation of agents in the context of reinforcement learning concepts. 
# Users can implement their own custom agents within this module.
...
from algorithm.local_predictor import LocalCkptPredictor as LocalPredictor
from algorithm.utils import cvt_tensor_to_infer_input, cvt_tensor_to_infer_output
from conf.config import ModelConfig, Config
...
class RandomAgent():
    '''
    A simple implementation of a random action output agent, 
    which can be used for process testing or expanded into a heuristic AI implementation.
    
    一个随机输出动作Agent的简单实现，可以用作流程测试或者拓展为启发式AI实现
    '''
...
class Agent:
    '''
    Implementation of agents in the context of reinforcement learning concepts, 
    including the model and model decision interface.
    ...
    def __init__(self, model_cls, model_pool_addr, keep_latest=False, local_mode=False, dataset=None):
        ...
        # lstm_* is used for LSTM information storage, 
        # providing the agent with the memory capability for making decisions based on long sequences.
        ...
        # Output decision information, including decision dimensions and legal action.
        # 输出决策的信息，包括决策维度和合法动作
        ...
        # Whether to load an existing dataset for training, with the default option being not to load.
        # 是否载入现有数据集训练，默认不载入
        if dataset is None:
    ...
    def _update_model_list(self):
        '''
        Update the list of model files synchronized from the learner.
    ...
    def load_last_new_model(self, models_path):
        '''
        Load the latest model file synchronized from the learner.
    ...
    def _load_model(self, model_version):
        '''
        Implement a method for loading the model, which is not intended to be called externally in principle.
    ...
    def _get_random_model(self):
        '''
        load a random model from the model list.
    ...
    def _predict_process(self, feature, legal_action,lstm_cell,lstm_hidden):
        '''
        Inference process, including data interface conversion.

        infer流程，包括数据接口的转换
        '''

        # split the data of 5 heroes
        # 将5个英雄的数据分离
    ...
    def _sample_masked_action(self, logits, legal_action):
        ...
        # deal with the last prediction, target
        # 处理最后的预测和目标
