...
# Module Introduction: This module encapsulates the creation and training operations of the model, 
# as well as the operations for interaction between the model and the environment.
...
from framework.server.learner.gradient_fusion import *
from algorithm.ppo.model import Model
...
class Graphs(object):
    '''
    Model encapsulation class, build_model and _add_forward_pass_and_gradients are the core logic. 
    Obtaining tf.ops.
    ...
    def _add_forward_pass_and_gradients(self, datas):
        '''
        Create ops for storing intermediate copies, loss, gradients, etc.
...
class GameController(object):
    '''
    GameController is the specific algorithm class for the "sgame_1v1". Both the actor and learner will invoke it, 
    but they have different behaviors in certain parts. To differentiate between them, the model parameter is used.
    
    GameController是sgame_1v1的具体算法类, actor和learner都会调用,由于两者行为部分行为不同, 采用model参数来进行区分
    '''
    ...
    def __init__(self, network=None, name="sgame", mode="actor"):
        '''
        Initialization function of the Model class.
        Args:
            network: An instance of the custom Network class for "sgame_1v1".
            name: model name
            mode: pattern of use, actor or learner
    ...
    '''
        ...
        # For the PPO algorithm, set the number of epochs and increase sample utilization. 
        # Additionally, PPO is more suitable for multiple updates in the current on-policy sampling mode, which can be modified according to the business requirements.
        # 针对PPO算法, 设置epoch次数, 增加样本利用率，同时ppo在当前的on-policy采样模式下更适合多次更新, 业务可以修改
        self.ppo_epoch = 3
    ...
    def on_init(self):
        '''
        Initialization operations before starting the game.
    ...
    def create_agent(self):
        '''
        make sure that the modelpoll is working before starting the game.
        ...
        with tf.device(self.cpu_device):
            ...
            with tf.control_dependencies([main_fetch_group]):
                ...
                with tf.compat.v1.variable_scope("summary"):
                    tf.compat.v1.summary.scalar("loss_all", info_list['all_loss_value']),
                    tf.compat.v1.summary.scalar("value_loss", info_list['value_loss']),
                    tf.compat.v1.summary.scalar("policy_loss", info_list['policy_loss']),
                    tf.compat.v1.summary.scalar("entropy_loss", info_list['entropy_loss']),
                    tf.compat.v1.summary.scalar("ratio_mean", info_list['ratio_mean']),
                    tf.compat.v1.summary.scalar("policy_log_p_mean", info_list['policy_log_p_mean']),
                    tf.compat.v1.summary.scalar("old_policy_log_p_mean", info_list['old_policy_log_p_mean']),
    ...
    def init_model(self):
        ...
        # TensorFlow officially recommends using tf.compat.v1.train.MonitoredTrainingSession.
        ...
        # Print network values and weights.
        # 打印网络值和权重
    ...
    def timeline_after_run(self, step, steps, run_metadata):
        if step in steps:
            ...
            # After this execution is completed, set print_timeline to False.
    ...
    def proflie_after_run(self, step, run_options, run_metadata, trim_name_regexes=None):
        ...
        if step >= CONFIG.print_profile_end_step:
            ...
            # After this execution is completed, set print_profile to False.
            # 本次执行完成后, 设置print_profile为False
            CONFIG.print_profile = False

    def predict(self, state_dict):
        '''
        Perform forward propagation of the actor to make predictions.
        Args:
            state_dict: The state information required for prediction is defined by the state class of sgame_1v1.
        ...
        返回: format_actions, network_sample_info
    ...
    def on_predict(self, observation, legal_action, sub_action_mask, lstm_hidden, lstm_cell, **kwargs):
        '''
        The real called predict function.
    ...
    def _format_actions(self, actions):
        ...
        检查动作是否在定义的范围内，并将其格式化为gamecore的动作
        """
    ...
    def get_must_sample_info(self, agent, pred_ret, state_legal_action):
        """
        Get the information required to generate samples.
    ...
    def train(self):
        ...
        with TimeIt() as ti:
            ...
            for _ in range(self.ppo_epoch):
                if self.is_chief_rank:
                    ...
                    # Set the run options to "full trace".
                    # 设置run options为full trace
        ...
        self.logger.info(f'sgame_learner train cost {batch_duration * 1000} ms')

        return results['info_list']
   
    def should_stop(self):
        '''
        The determination of whether the game environment can be stopped depends on the requirements and needs to be set accordingly.
